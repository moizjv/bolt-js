...
export interface ActionConstraints<A extends SlackAction = SlackAction> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  callback_id?: Extract<A, { callback_id?: string }> extends any
    ? string | RegExp
    : never;
}
...
export default class App<
  AppCustomContext extends StringIndexed = StringIndexed
> {
  ...
  public constructor({
    ...
    if (
      typeof this.logLevel !== "undefined" &&
      this.logger.getLevel() !== this.logLevel
    ) {
    ...
    if (
      socketMode &&
      port !== undefined &&
      this.installerOptions.port === undefined
    ) {
    ...
    if (
      ...
      this.installerOptions.callbackOptions = {
        failure: (error, _installOptions, _req, res) => {
          ...
          res.end(
            `<html><body><h1>OAuth failed!</h1><div>${escapeHtml(
              error.code
            )}</div></body></html>`
          );
        },
    ...
    if (convoStore !== false) {
      ...
      const store: ConversationStore =
        convoStore === undefined ? new MemoryStore() : convoStore;
  ...
  public message<MiddlewareCustomContext extends StringIndexed = StringIndexed>(
    ...listeners: MessageEventMiddleware<
      AppCustomContext & MiddlewareCustomContext
    >[]
  ): void;
  /**
  ...
  public message<MiddlewareCustomContext extends StringIndexed = StringIndexed>(
    ...
    const messageMiddleware = patternsOrMiddleware.map(
      (patternOrMiddleware) => {
        if (
  ...
  >(
    ...
    ...listeners: Middleware<
      SlackShortcutMiddlewareArgs<
        Extract<Shortcut, { type: Constraints["type"] }>
      >,
      AppCustomContext & MiddlewareCustomContext
    >[]
  ): void;
  ...
  >(
    ...
    if (
      ...
      !validViewTypes.includes(constraints.type)
    ) {
      this.logger.error(
  ...
  public async processEvent(event: ReceiverEvent): Promise<void> {
    ...
    if (
      ...
      isEventTypeToSkipAuthorize(event.body.event.type)
    ) {
      authorizeResult = {
      ...
      try {
        ...
        this.logger.warn(
          "Authorization of incoming event did not succeed. No listeners will be called."
        );
    ...
    if (
      authorizeResult.enterpriseId === undefined &&
      source.enterpriseId !== undefined
    ) {
    ...
    if (typeof event.customProperties !== "undefined") {
      ...
      const builtinKeyDetected = contextBuiltinKeys.find(
        (key) => key in customProps
      );
      if (typeof builtinKeyDetected !== "undefined") {
        throw new InvalidCustomPropertyError(
          "customProperties cannot have the same names with the built-in ones"
        );
      }
    }
    ...
    switch (type) {
      ...
      case IncomingEventType.Action:
        if (
          ...
          const { actions } = bodyArg as SlackActionMiddlewareArgs<
    ...
    if (type === IncomingEventType.Event) {
      ...
      if (eventListenerArgs.event.type === "message") {
        const messageEventListenerArgs =
          eventListenerArgs as SlackEventMiddlewareArgs<"message">;
      ...
      const optionListenerArgs =
        listenerArgs as SlackOptionsMiddlewareArgs<OptionsSource>;
    ...
    if (body.response_url) {
      ...
      typeof body.response_urls !== "undefined" &&
      body.response_urls.length > 0
    ) {
      // This can exist only when view_submission payloads - response_url_enabled: true
      listenerArgs.respond = buildRespondFn(
        this.axios,
        body.response_urls[0].response_url
      );
    }
      ...
      await processMiddleware(
        ...
        async () => {
          ...
          if (rejectedListenerResults.length === 1) {
            ...
            throw new MultipleListenerError(
              rejectedListenerResults.map((rlr) => rlr.reason)
            );
          }
        }
      );
  ...
  private initAuthorizeIfNoTokenIsGiven(
    ...
    if (authorize === undefined && !usingOauth) {
      ...
      throw new AppInitializationError(
        `You cannot provide both authorize and oauth installer options. ${tokenUsage}`
      );
    } else if (authorize === undefined && usingOauth) {
...
function buildSource<IsEnterpriseInstall extends boolean>(
  ...
  const enterpriseId: string | undefined = (() => {
    ...
    if (
      ...
      if (
        typeof bodyAsActionOrOptionsOrViewActionOrShortcut.enterprise !==
          "undefined" &&
      ...
      // When the app is installed using org-wide deployment, team property will be null
      if (
        typeof bodyAsActionOrOptionsOrViewActionOrShortcut.team !==
          "undefined" &&
  ...
  return {
    userId,
    isEnterpriseInstall,
    teamId: teamId as IsEnterpriseInstall extends true
      ? string | undefined
      : string,
    enterpriseId: enterpriseId as IsEnterpriseInstall extends true
      ? string
      : string | undefined,
...
function buildRespondFn(
  ...
  return async (message: string | RespondArguments) => {
    const normalizedArgs: RespondArguments =
